...
def PricePredictor(to_predict_list):
    ...
    loaded_model = load('app/model.joblib')
    result = loaded_model.predict(to_predict)
    return result[0]


def BuildVisuals():
    df = pd.read_csv('app/toyota-cars-temp.csv')
    ...
    fig.write_image("app/static/datadistribution.svg", width=500)
    correlation = df_cur.corr()
    fig_cor = px.imshow(correlation)
    fig_cor.write_image("app/static/correlation_hist.svg", width=500)
    ...
    fig_features.write_image("app/static/feature_importances.svg", width=500)
